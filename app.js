...
//Database connection..
const db_url =
  "mongodb+srv://local_library_user:local_library@cluster0-nhau9.azure.mongodb.net/capodicapi?retryWrites=true&w=majority";
...
io.on("connection", (client) => {
  ...
  client.on("changeUsername", (data) => {
    if (data.username == "") {
      ...
      sendNotification(
        "Username Changed",
        `${client.username}`,
        `${oldname} is now ${client.username}`
      );
    }
  });
  ...
  client.on("history", (data) => {
    ...
    if (data === "") {
      ...
      async function history() {
        const result = await Chat.find({ sender: data })
      ...
      async function logs() {
        const result = await Log.find({ name: data })
          .limit(10)
          .sort({ date: -1 });
  ...
  client.on("room1", (room) => {
    ...
    client.emit("privateWelcome", {
      welcomeMsg: `Welcome to ${room.currentRoom}`,
    });
    logSave({
      logtype: "New Room",
      name: room.name,
